		   ...
		   // collisionMap.draw3D(g);
			...
			if(alive) {
				...
				case "en1": 
					...
					if(!enabled) g.drawImage(animator.get("moving").getStillImage(),x,y,sw,sh,null);
					break;
				case "en2":
					g.drawImage(animator.get("2moving").getCurrentImage(),x,y,sw,sh,null); 
				    if(!enabled) g.drawImage(animator.get("2moving").getStillImage(),x,y,sw,sh,null);
				    break;
				case "met":
					g.drawImage(animator.get("met").getCurrentImage(),x,y,sw,sh,null); 
					if(!enabled) g.drawImage(animator.get("met").getStillImage(),x,y,sw,sh,null);
					break;
				default:
					break;
				}
