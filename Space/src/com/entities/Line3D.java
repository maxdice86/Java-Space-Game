...
public class Line3D
{
   ...
   double zClip = 1;
	...
	public void draw(Graphics g)
	{
		...
		g.drawLine
		(
			(int)(x1 - Camera3D.x) / scale + x_origin, 
			(int)(z1 - Camera3D.z) / scale + y_origin, 
			(int)(x2 - Camera3D.x) / scale + x_origin, 
			(int)(z2 - Camera3D.z) / scale + y_origin
		);
	}
...
//*/
	...
	public void grabbedAt(int mx, int mz)
	{
		...
		if(dx*dx + dz*dz < 49)  held_at = 1;
		
		dx = x2 - mx;
		dz = z2 - mz;
		
		if(dx*dx + dz*dz < 49)  held_at = 2;
	}
